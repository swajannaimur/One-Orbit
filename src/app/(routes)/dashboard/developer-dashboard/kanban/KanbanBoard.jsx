                ...
                {(provided, snapshot) => (
                  ...
                  >
                    {(tasksByStatus[col.key] || []).map((task, index) => {
                      ...
                      // NEW (Corrected)
                      const stableId = normalizeId(task?._id ?? task?.id);
                      if (!stableId) {
                        // Optionally, log an error for tasks missing an ID after normalization
                        console.error(
                          "Task rendered without a stable ID:",
                          task
                        );
                        return null; // Don't render an un-draggable item
                      }
