...
export const POST = async (req) => {
    try {
        ...
        const projectName = req.headers.get("projectName");
        const developerEmail = req.headers.get("devEmail");

        if (!projectId || !developerEmail || !projectName) {
            return NextResponse.json({ success: false, error: "Missing required headers" });
        }
        ...
        const existingProject = await collection.findOne({ projectId: mongoProjectId });
        ...
        if (existingProject) {
            ...
            // jodi developer ta already not exist 
                ...
                const postsDocument = await postsCollection.findOne({ _id: mongoProjectId })
                if(!postsDocument) {
                    return NextResponse.json({success: false, error: "No matching post found with this id"});
                } 
                ...
                await postsCollection.updateOne(
                    { projectId: mongoProjectId },
                    { $set: { assigned: true, members: [...(existingProject.developers || []), developerEmail] } },
                    { upsert: true }
                );

                return NextResponse.json({ success: true, message: "Developer added to existing project" });
            }
        } else {
            ...
            return NextResponse.json({ success: true, insertedId: result.insertedId, message: "New assigned record created" });
        }
